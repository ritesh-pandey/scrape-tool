/*
    Reads the already generated variant features file and checks if color 
    information is present or not.

    It generates 'missing_colors.json' file for variants with missing colors
        ...
        if (!err) {
            ...
            variants.forEach(function (oVariant) {
                if (!oVariant.discontinued) {
                    fs.readFile(modelRealPath + '/variants/' + oVariant.name.replace(/\ |\//g, '_') + '.json', {
                        encoding: 'utf-8'
                    }, function (err, featureJSONString) {
                        var featureJSON = JSON.parse(featureJSONString);
                        if (!featureJSON.colors) {
                            phantomQueue.push(oVariant.name);
                            fs.writeFile('missing_colors.json', JSON.stringify(phantomQueue, null, 2), function (err) {
